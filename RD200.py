...
- on change of either the Bq value or the error inform the client with a callback
        ...
        while not self.should_stop.is_set():
            if time.time() >= self.t_next_read:
                ...
                if Bq is None:
                    ...
                    error = False
                if (self.Bq != Bq) or (self.error != error):
                    self.Bq = Bq
                    self.error = error
                    self.on_change(self.Bq, self.error)
...
cnt = 0
def main():
    changes = 1 # will react immediatelly
    # changes = 2 # will need up to 10 minutes possibly plus n * 10 minutes if the value does not change
    # changes = 3 # will need up to 10 minutes plus (changes - 2) * 10 minutes plus n * 10 minutes if the value does not change
    def on_change(Bq, error):
        global cnt
        cnt += 1
        print(time.time(), "{}: {} Bq {}".format(cnt, Bq, "error" if error else ""))

    radon_reader = RD200(on_change)
